...
# Parameters
cbf_file_name = '2025-06-17_19-08-19_cbf_nonconvex_obstacle.json'
cbf_folder_name = r'Examples_without_symmetry/Data_large_files'
...
offset = 4.0
cbf_values = cbf_values + offset # Shifted CBF is still a CBF (see Wiltz et al. 2025)
...
domain_lower_bound = cbfModule.domain_lower_bound
domain_upper_bound = cbfModule.domain_upper_bound
domain_to_plot = [domain_lower_bound[0], domain_upper_bound[0], domain_lower_bound[1], domain_upper_bound[1]] # [x_min, x_max, y_min, y_max]
...
x_obstacle_grid = np.linspace(cbfModule.domain_lower_bound[0], cbfModule.domain_upper_bound[0], 1000)
y_obstacle_grid = np.linspace(cbfModule.domain_lower_bound[1], cbfModule.domain_upper_bound[1], 1000)
...
plt.contour(x_obstacle_grid, y_obstacle_grid, H_values, levels=[0], colors='k', alpha=1,  linewidths=2, linestyles='solid')      # plot the boundary of the obstacle with transparency
