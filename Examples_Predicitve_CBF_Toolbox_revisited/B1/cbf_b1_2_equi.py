"""
    ...
    Computation of the CBF for the more agile bicycle model with a circular obstacle using EQUIVARIANCES.
...
if __name__ == '__main__':
    ...
    cbf_file_name = "b1_2_cbfm_1p12_equi_reduced.json"
    
    # create a dynamic system
    t0 = 0
    x0 = np.array([0,0,0])
    L = 1
    u_min = np.array([1, -45/180*np.pi])    # [minimum speed, minimum steering angle] in [m/s, rad/s]
    u_max = np.array([2, 45/180*np.pi])     # [maximum speed, maximum steering angle] in [m/s, rad/s]
