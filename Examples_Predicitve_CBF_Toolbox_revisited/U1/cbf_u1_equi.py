"""
    ...
    Computation of the CBF for the unicycle with a circular obstacle using EQUIVARIANCES.
...
if __name__ == '__main__':
    ...
    cbf_file_name = "u1_2_cbfm_1p12_equi.json"
    ...
    # set domain bounds
    domain_lower_bound_partial = np.array([-14.2,0,-np.pi])
    domain_upper_bound_partial = np.array([0.1,0,0])
    discretization_partial = np.array([30,1,21])
    domain_lower_bound_partial_2 = np.array([-14.2,0,-np.pi])
    domain_upper_bound_partial_2 = np.array([0.1,0,np.pi])
    discretization_partial_2 = np.array([30,1,41])
    ...
    warmStartInputTrajectory_0 = np.array([u_min[0]*np.ones(myCBFmodule_partial.N), 
                                            u_max[1]*np.ones(myCBFmodule_partial.N)])   # max speed and turn to the right
    warmStartInputTrajectory_0[0,curve_steps:] = u_max[0]*np.ones(myCBFmodule_partial.N-curve_steps) # set the steering angle to zero after curve
    ...
    warmStartInputTrajectory_2 = np.array([u_min[0]*np.ones(myCBFmodule_partial.N),
                                            u_min[1]*np.ones(myCBFmodule_partial.N)])   # max speed and turn to the left
    warmStartInputTrajectory_2[0,curve_steps:] = u_max[0]*np.ones(myCBFmodule_partial.N-curve_steps) # set the steering angle to zero after curve
