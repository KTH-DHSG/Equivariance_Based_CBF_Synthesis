"""
    ...
    Computation of the CBF for the single integrator with a circular obstacle using EQUIVARIANCES.
    The here considered single integrator only allows for positive velocities in the x-direction.
...
if __name__ == '__main__':
    ...
    # some parameters
    num_of_batches_factor = 1      # determines the number of batches for parallel computation
    cbf_file_name = "s2_cbfm_equi.json"
    ...
    domain_lower_bound_partial = np.array([-15,0.0])
    domain_upper_bound_partial = np.array([0.1,0.0])
    discretization_partial = np.array([30,1])
    domain_lower_bound_complete = np.array([-10,-10])
    domain_upper_bound_complete = np.array([10,10])
    discretization_complete = np.array([41,41])

    print("Number of grid points to be explicitly computed: ", np.prod(discretization_partial))
    ...
    warmStartInputTrajectories_3 = np.array([u_min[0]*np.ones(myCBFmodule_partial.N),
